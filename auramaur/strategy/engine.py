...
class TradingEngine(CycleOrchestrationMixin):
    ...
    async def scan_and_store_markets(self, limit: int = 300) -> list[Market]:
        ...
        for market in markets:
            await self.db.execute(
                ...
                "(market_id, exchange, price, recorded_at, snapshot_key) "
                "VALUES (?, ?, ?, datetime('now'), ?)",
                (market.id, market.exchange or self.exchange_name or "polymarket",
                 market.outcome_yes_price,
                 f"{market.exchange or self.exchange_name or 'polymarket'}:{market.id}:"
    ...
    async def analyze_market(
        ...
        *,
        ...
        try:
            await self.db.execute("SAVEPOINT forecast_observation")
            await self.db.execute(
            ...
            await self.db.execute("RELEASE SAVEPOINT forecast_observation")
            await self.db.commit()
        except Exception:
            try:
                await self.db.execute("ROLLBACK TO SAVEPOINT forecast_observation")
                await self.db.execute("RELEASE SAVEPOINT forecast_observation")
            finally:
