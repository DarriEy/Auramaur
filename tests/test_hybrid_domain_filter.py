...
class TestGetWhitelistCategories:
    def test_cold_start_returns_empty(self, feedback, local_loop):
        """With no calibration data, whitelist and probationary are both empty."""
        wl, prob = run(feedback.get_whitelist_categories(), local_loop)
        assert wl == set()
        assert prob == set()

    def test_category_whitelisted_when_accurate(self, feedback, db, local_loop):
        """Category with >=50% accuracy and >=10 trades is whitelisted."""
        run(_seed_calibration(db, "politics", correct=8, incorrect=4), local_loop)
        wl, prob = run(feedback.get_whitelist_categories(min_accuracy=0.50, min_trades=10), local_loop)
        # 12 trades, 8/12 = 66.7% accuracy -> whitelisted
        assert "politics" in wl
        assert "politics" not in prob

    def test_category_probationary_when_insufficient_trades(self, feedback, db, local_loop):
        """Category with good accuracy but <10 trades is probationary."""
        run(_seed_calibration(db, "sports", correct=4, incorrect=1), local_loop)
        wl, prob = run(feedback.get_whitelist_categories(min_accuracy=0.50, min_trades=10), local_loop)
        assert "sports" not in wl
        assert "sports" in prob

    def test_category_probationary_when_low_accuracy(self, feedback, db, local_loop):
        """Category with enough trades but <50% accuracy is probationary."""
        run(_seed_calibration(db, "crypto", correct=3, incorrect=9), local_loop)
        wl, prob = run(feedback.get_whitelist_categories(min_accuracy=0.50, min_trades=10), local_loop)
        assert "crypto" not in wl
        assert "crypto" in prob

    def test_multiple_categories(self, feedback, db, local_loop):
        """Mixed categories are correctly split between whitelisted and probationary."""
        run(_seed_calibration(db, "politics", correct=8, incorrect=4), local_loop)
        run(_seed_calibration(db, "crypto", correct=3, incorrect=9), local_loop)
        run(_seed_calibration(db, "tech", correct=2, incorrect=1), local_loop)
        wl, prob = run(feedback.get_whitelist_categories(min_accuracy=0.50, min_trades=10), local_loop)
        assert wl == {"politics"}
        assert prob == {"crypto", "tech"}

    def test_custom_thresholds(self, feedback, db, local_loop):
        """Custom min_accuracy and min_trades are respected."""
        run(_seed_calibration(db, "science", correct=4, incorrect=1), local_loop)
        wl, prob = run(feedback.get_whitelist_categories(min_accuracy=0.50, min_trades=5), local_loop)
