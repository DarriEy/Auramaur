...
class TestCalibrationTracker:
    def test_adjust_no_params_returns_raw(self, tracker, local_loop):
        """With no fitted params, adjust() should return the raw probability."""
        result = run(tracker.adjust(0.7, "politics"), local_loop)
        assert result == 0.7

    def test_fit_params_insufficient_data(self, tracker, local_loop):
        ...
        for i in range(3):
            ...
            run(tracker.record_resolution(f"mkt_{i}", True), local_loop)

        result = run(tracker.fit_params(category="politics"), local_loop)
        assert result is None

    def test_fit_params_with_data(self, tracker, local_loop):
        ...
        for i in range(10):
            ...
            run(tracker.record_prediction(f"mkt_{i}", predicted, ""), local_loop)
            run(tracker.record_resolution(f"mkt_{i}", actual), local_loop)

        result = run(tracker.fit_params(category=None), local_loop)
        assert result is not None
        a, b = result
        assert isinstance(a, float)
        assert isinstance(b, float)

    def test_adjust_with_bias_correction(self, tracker, local_loop):
        ...
        run(tracker.fit_params(category=None), local_loop)

        # Now test: a prediction of 0.65 should be adjusted downward
        calibrated = run(tracker.adjust(0.65), local_loop)
        assert calibrated < 0.65, (
            f"Expected calibrated ({calibrated:.3f}) < raw (0.65) for overconfident predictor"
        )

    def test_per_category_calibration(self, tracker, local_loop):
        """Per-category params should be used when available."""
        # Politics: predictions are too high
        for i in range(10):
            run(tracker.record_prediction(f"pol_{i}", 0.8, "politics"), local_loop)
            run(tracker.record_resolution(f"pol_{i}", i < 5), local_loop)

        # Sports: predictions are too low
        for i in range(10):
            run(tracker.record_prediction(f"spt_{i}", 0.3, "sports"), local_loop)
            run(tracker.record_resolution(f"spt_{i}", i < 7), local_loop)

        # Fit both categories
        run(tracker.fit_params(category="politics"), local_loop)
        run(tracker.fit_params(category="sports"), local_loop)

        pol_adjusted = run(tracker.adjust(0.8, "politics"), local_loop)
        spt_adjusted = run(tracker.adjust(0.3, "sports"), local_loop)
    ...
    def test_category_fallback_to_global(self, tracker, local_loop):
        """Unknown category should fall back to global params."""
        # Fit global params only
        for i in range(10):
            run(tracker.record_prediction(f"mkt_{i}", 0.7, ""), local_loop)
            run(tracker.record_resolution(f"mkt_{i}", i < 4), local_loop)

        run(tracker.fit_params(category=None), local_loop)

        # Query with unknown category should use global params
        result = run(tracker.adjust(0.7, "unknown_category"), local_loop)
        assert result != 0.7  # Should be adjusted (not raw)

    def test_refit_all(self, tracker, local_loop):
        """refit_all should fit global and qualifying categories."""
        # Global data
        for i in range(10):
            run(tracker.record_prediction(f"g_{i}", 0.6, ""), local_loop)
            run(tracker.record_resolution(f"g_{i}", i < 5), local_loop)

        # Category with enough data
        for i in range(8):
            run(tracker.record_prediction(f"p_{i}", 0.7, "politics"), local_loop)
            run(tracker.record_resolution(f"p_{i}", i < 4), local_loop)

        # Category with too little data
        for i in range(3):
            run(tracker.record_prediction(f"s_{i}", 0.5, "sports"), local_loop)
            run(tracker.record_resolution(f"s_{i}", True), local_loop)

        run(tracker.refit_all(), local_loop)

        # Global should be fitted
        global_result = run(tracker.adjust(0.6, ""), local_loop)
        assert global_result != 0.6

        # Politics should be fitted
        pol_result = run(tracker.adjust(0.7, "politics"), local_loop)
        assert pol_result != 0.7

    def test_record_prediction_with_category(self, tracker, local_loop):
        """record_prediction should store category."""
        run(tracker.record_prediction("mkt_1", 0.6, "crypto"), local_loop)
    ...
    def test_brier_score(self, tracker, local_loop):
        """Brier score should work with the updated schema."""
        run(tracker.record_prediction("m1", 0.9, ""), local_loop)
        run(tracker.record_resolution("m1", True), local_loop)
        run(tracker.record_prediction("m2", 0.9, ""), local_loop)
        run(tracker.record_resolution("m2", False), local_loop)

        score = run(tracker.get_brier_score(), local_loop)
        assert score is not None
        # (0.9-1)^2 + (0.9-0)^2 / 2 = (0.01 + 0.81) / 2 = 0.41
        assert abs(score - 0.41) < 0.01

    def test_backfill_resolution_pnl_from_live_fills(self, db, local_loop):
        ...
        result = run(backfill_resolution_pnl(db, dry_run=False), local_loop)
        run(rebuild_category_stats_from_resolution_pnl(db, dry_run=False), local_loop)
...
class TestOnlineCalibration:
    ...
    def test_online_update_modifies_params(self, tracker, local_loop):
        """online_update should modify cached Platt scaling parameters."""
        # First fit initial params with batch data
        for i in range(10):
            run(tracker.record_prediction(f"mkt_{i}", 0.7, ""), local_loop)
            run(tracker.record_resolution(f"mkt_{i}", i < 5), local_loop)
        ...
        run(tracker.online_update(0.8, 0.0, ""), local_loop)
    ...
    def test_online_update_direction(self, tracker, local_loop):
        """Online update after overconfident prediction should shift params."""
        # Fit initial params
        for i in range(10):
            run(tracker.record_prediction(f"mkt_{i}", 0.6, ""), local_loop)
            run(tracker.record_resolution(f"mkt_{i}", i < 6), local_loop)

        run(tracker.fit_params(category=None), local_loop)
        before = run(tracker.adjust(0.9, ""), local_loop)

        # We predicted 0.9 but outcome was NO (0.0) — overconfident
        run(tracker.online_update(0.9, 0.0, ""), local_loop)

        after = run(tracker.adjust(0.9, ""), local_loop)
    ...
    def test_record_resolution_triggers_online_update(self, tracker, local_loop):
        """record_resolution should automatically call online_update."""
        # Fit initial params
        for i in range(10):
            run(tracker.record_prediction(f"init_{i}", 0.6, ""), local_loop)
            run(tracker.record_resolution(f"init_{i}", i < 6), local_loop)

        run(tracker.fit_params(category=None), local_loop)
        old_params = tracker._params_cache[""]

        # Record a new prediction + resolution
        run(tracker.record_prediction("new_mkt", 0.85, ""), local_loop)
        run(tracker.record_resolution("new_mkt", False), local_loop)
    ...
    def test_online_update_persists_to_db(self, tracker, local_loop):
        """Online update should persist new params to database."""
        # Fit initial params
        for i in range(10):
            run(tracker.record_prediction(f"mkt_{i}", 0.5, ""), local_loop)
            run(tracker.record_resolution(f"mkt_{i}", i < 5), local_loop)

        run(tracker.fit_params(category=None), local_loop)

        # Do online update
        run(tracker.online_update(0.8, 1.0, ""), local_loop)
    ...
    def test_online_update_category_specific(self, tracker, local_loop):
        """Online update with category should update both category and global."""
        # Fit global and category params
        for i in range(10):
            run(tracker.record_prediction(f"g_{i}", 0.6, ""), local_loop)
            run(tracker.record_resolution(f"g_{i}", i < 6), local_loop)
        for i in range(10):
            run(tracker.record_prediction(f"p_{i}", 0.7, "politics"), local_loop)
            run(tracker.record_resolution(f"p_{i}", i < 4), local_loop)
        ...
        run(tracker.online_update(0.8, 0.0, "politics"), local_loop)
    ...
    def test_moving_brier_score_single(self, tracker, local_loop):
        ...
        run(tracker.online_update(0.9, 1.0, ""), local_loop)
    ...
    def test_moving_brier_score_window(self, tracker, local_loop):
...
class TestSnapshotDedup:
    ...
    def test_brier_dedups_snapshots(self, db, tracker, local_loop):
        # m1 snapshotted 5x (0.2, YES); m2 once (0.8, YES).
        run(self._seed(db, "m1", 0.2, 1, n=5), local_loop)
        run(self._seed(db, "m2", 0.8, 1, n=1), local_loop)
        score = run(tracker.get_brier_score(), local_loop)
        # Deduped: mean((0.2-1)^2, (0.8-1)^2) = (0.64 + 0.04) / 2 = 0.34.
        # Per-snapshot it would be (5*0.64 + 0.04) / 6 = 0.54.
        assert abs(score - 0.34) < 1e-9

    def test_fit_counts_distinct_markets(self, db, local_loop):
        tracker = CalibrationTracker(db=db, min_samples=10)
        # 15 snapshots but ONE market — below the 10 distinct-market threshold.
        run(self._seed(db, "m1", 0.6, 1, n=15), local_loop)
        assert run(tracker.fit_params(category=None), local_loop) is None
