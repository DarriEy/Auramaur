...
class TestSnapshotDedup:
    def test_snapshots_of_one_market_count_once(self, db, feedback, local_loop):
        """One market snapshotted 12 times is a single resolution event."""
        for _ in range(12):
            run(_add(db, "m1", 0.2, 1, "sports"), local_loop)  # predicted NO, resolved YES

        stats = run(feedback.get_category_accuracy(), local_loop)
        ...
        avoid = run(feedback.get_avoid_categories(), local_loop)
        assert "sports" not in avoid

    def test_avoid_fires_on_distinct_bad_markets(self, db, feedback, local_loop):
        """12 distinct markets all called wrong → category is avoided."""
        for i in range(12):
            run(_add(db, f"bad{i}", 0.2, 1, "badcat"), local_loop)
    ...
    def test_dedup_keeps_latest_prediction(self, db, feedback, local_loop):
        """When a market is re-forecast, the most recent snapshot wins."""
        run(_add(db, "m1", 0.1, 1, "tech"), local_loop)  # early: wrong
        run(_add(db, "m1", 0.9, 1, "tech"), local_loop)  # latest: right

        stats = run(feedback.get_category_accuracy(), local_loop)
        assert stats["tech"]["trade_count"] == 1
        assert stats["tech"]["accuracy"] == 1.0  # latest snapshot is correct

    def test_whitelist_counts_distinct_markets(self, db, feedback, local_loop):
        ...
        for _ in range(12):
            run(_add(db, "g1", 0.9, 1, "crypto"), local_loop)  # one market, called right
